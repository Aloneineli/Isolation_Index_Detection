...
def find_most_anomalous_rows_knn(X, k=5, n_anomalous=1):
    nbrs = NearestNeighbors(n_neighbors=k + 1).fit(X)
    distances, indices = nbrs.kneighbors(X)
    anomaly_scores = np.mean(distances[:, 1:], axis=1)
    most_anomalous_indices = np.argsort(-anomaly_scores)[:n_anomalous]
    most_anomalous_rows = X[most_anomalous_indices]
    return most_anomalous_rows, anomaly_scores[most_anomalous_indices]


def find_optimal_k_knn(X, k_values, n_anomalous=1):
    best_k = None
    best_score = -np.inf
    best_row = None

    for k in k_values:
        rows, scores = find_most_anomalous_rows_knn(X, k=k, n_anomalous=n_anomalous)
...
if __name__ == '__main__':
    ...
    # Find the optimal k
    optimal_k, optimal_row = find_optimal_k_knn(X, k_values, n_anomalous=1)
