...
def find_most_anomalous_rows_loda(X, k=5, n_anomalous=1):
    # Ensure the matrix X is of type float64
    X = X.astype(np.float64)

    # Step 1: Fit the LODA model
    clf = LODA(n_bins=k)
    clf.fit(X)

    # Step 2: Compute the LODA scores
    loda_scores = clf.decision_scores_

    # Step 3: Identify the most anomalous rows
    most_anomalous_indices = np.argsort(-loda_scores)[:n_anomalous]
    most_anomalous_rows = X[most_anomalous_indices]

    return most_anomalous_rows, loda_scores[most_anomalous_indices]


def find_optimal_k_loda(X, k_values, n_anomalous=1):
    best_k = None
    best_score = -np.inf
    best_row = None

    for k in k_values:
        rows, scores = find_most_anomalous_rows_loda(X, k=k, n_anomalous=n_anomalous)
...
if __name__ == '__main__':
    ...
    # Find the optimal k
    optimal_k, optimal_row = find_optimal_k_loda(X, k_values, n_anomalous=1)
